    ...
    <div class="container">
        <form action="{{ route('Nav.update',['shop'=>$Nav]) }}" method="post" enctype="multipart/form-data">
            <div class="form-group">
                ...
                <input type="text" name="name" value="@if(old('name')){{ old('name') }}@else{{ $Nav->name }}@endif" class="">
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <select name="permission_id" id="">
                    @foreach($permission as $val)
                        <option @if($Nav->permission_id==$val->id) selected @endif value="{{ $val->id }}">{{$val->name}}</option>
                    @endforeach
                </select>
            </div>
            {{ method_field('PATCH') }}
